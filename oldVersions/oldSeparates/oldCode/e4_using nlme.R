...
#Analysis of uplanted and monoculture pots, Parts 1 and 2 - Summarize and plot data
par(mfrow = c(2, 2)) #number of panels
par(cex = 0.8) #font size
par(mar = c(0, 4, 0, 0), oma = c(5, 5, 0.5, 0.5)) #size of inner, outer margins 
...
resps<-datamf[,c(1:4)]
View(resps)
respnames<-c('Community biomass (g)','Soil moisture (%)','Nitrate (ugN/G)','Net Nitrification (ugN/G*d)')
...
#Analysis of mixed community pots, Part 1 - Summarize data
...
###########################################
###########################################
#Analysis of mixed community pots, Part 2 - plot parameters to determine if correlated: logtotal/mvtrt, logtotal/comptrt
data<-dataaf
...
arrows(as.numeric(vec)+.1,mean+se,     				#error bars
       ...
       angle=90, code=3, length=0, col=1) 
axis(1, at = 1:3, label=c('No comp','P.v.','S.b.'))
mtext(letters[2], side = 3, line = -1.5, adj = 0.05, cex = 1, font=2) 
box()
mtext('Log-transformed \n Community biomass', side = 2, outer = T, cex = 1, line=2.2, adj=0.7)
mtext("Competitor treatment", side = 1, outer = F, cex = 1, line=2.2)
#export 2-panel figure - param_correl.png

###########################################
###########################################
#Analysis of mixed community pots, Parts 3 and 4 - Make figure 3 and eval model
...
xlim2 <- c(1.9,5.1)
resps<-dataaf[,c(1:3)]
respnames<-c('Soil moisture (%)','Nitrate (ugN/G)','Nitrification (ugN/G*d)')
...
for (i in 1:dim(resps)[2]){
  ...
  if (i %in% 3) {
    axis(1)
    mtext('Log-transformed \n Community biomass', side = 1, outer = F, cex = 1, line=3.2)}
  
  if (i %in% 1) {
    legend(x=5.25,y=107.25, 
           legend=c("No comp", "P.v.","S.b.", NA,
                    "No comp fit","P.v. fit","S.b. fit", NA,
                    "Comm. \nbiomass fit", NA), 
           cex=.8,
...
###########################################
###########################################
#Analysis of mixed community pots...con't

#Analysis of Mvtrt on measured Mv abundance
#1. plot of mvabund v mvtrt, mvrelabund v mvtrt
#2a. lme4 model: mvabund<-mvtrt+(1|bk) and test of fixed effects
#2a. lme4 model: relmvabund<-mvtrt+(1|bk) and test of fixed effects

#Part 1 - Plot
par(mfrow = c(2, 1)) #number of panels
par(cex = 0.8) #font size
par(mar = c(0, 4, 0, 0), oma = c(5, 5, 0.5, 8)) #size of inner, outer margins 
par(tcl = -0.25) #length of tick marks
par(mgp = c(2, 0.6, 0))
...
legend(x=5.75,y=114, 
       legend=c('No comp','P.v.','S.b.'), 
       pch=pchs, lty=ltys, col=cols, cex=.8, 
       y.intersp=1.5,  
       xpd=NA)
mtext('b', side = 3, line = -1.5, adj = 0.05, cex = 1, font=2) 
axis(2)
axis(1)
mtext('M.v. relative abundance (%)', side = 2, outer = F, cex = 1, line=2.2)
mtext('M.v. density treatment \n (# M.v. inds)', side = 1, outer = F, cex = 1, line=3.2)
