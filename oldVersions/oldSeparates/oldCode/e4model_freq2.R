#e4model_freq2.R -- model includes total plant biomass
# 3/26/13
setwd("~/Desktop/E4_R")
...
##################################
#Model structure
# response ~ totalbiomass* Mvtrt * comptrt * Mvtrtxcomptrt
#where...
...
for (i in 1:dim(response)[2]){ #loop through each response
  ...
  model.lme<-lme(fixed=responsenow ~ logtotal+mvtrt + comptrt + mvtrt*comptrt, random=~1|bk, data=data1, method="ML") #define the model, Model type 2 - Frequentist version with block effects
...
write.table(table.anovas.lme, file='model2_mv1to5.txt', sep='\t', row.names=T, col.names=T)


#######################################
#Nested models
#Nodi
data1 <- data.frame(nodi, logtotal, mvtrt, comptrt, bk) #define the dataframe
lme.model<-lme(nodi~ logtotal+mvtrt+comptrt, random = ~1|bk, data=data1)
anova(lme.model)
lme.posthocs <- glht(lme.model, linfct=mcp(comptrt="Tukey")) 
...
tmp<-sqrt(datam$total+5) #transform total
hist(tmp)
shapiro.test(tmp) #almost normal
datam$logtotal<-sqrt(datam$total+5)

logtotal<-datam$logtotal #x1
mvtrt<-datam$Mvtrt #x2
comptrt<-datam$comptrt #x3
bk<-datam$bk #block
#ys
...
write.table(table.anovas.lme, file='model2_mv0to5.txt', sep='\t', row.names=T, col.names=T)


#######################################
#Nested models
#Soilmoi
data1 <- data.frame(soilmoi, logtotal, mvtrt, comptrt, bk) #define the dataframe
lme.model<-lme(soilmoi~ logtotal+mvtrt+comptrt+mvtrt*comptrt, random = ~1|bk, data=data1)
anova(lme.model)
