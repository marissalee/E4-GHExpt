...
for (i in (2:6)){
  if(i!=6){
  	...
  	points(x=x[ME], y=ys[ME,i], pch=1)
  	points(x=x[M], y=ys[M,i], pch=2)
...
for (i in (1:6)){
  if(i!=6){
  	...
  	points(x=x[P], y=ys[P,i], pch=19,col='grey56')
  	points(x=x[MP], y=ys[MP,i], pch=2, col='grey56')
  	points(x=x[M], y=ys[M,i], pch=19)
  	lines(stats::lowess(data.frame(x=x[c(P,MP,M)],y=ys[c(P,MP,M),i])), lty=1)
  	}
 if(i==6){
 	par(mar=c(5,1,1,1))
...
for (i in (1:6)){
  if(i!=6){
  	...
  	axis(1, at=AT, tick=T, labels=F, tcl=-.2)
  	axis(2, at=ATys[[i]], tick=T, labels=F, tcl=-.2)
  	points(x=x[S], y=ys[S,i], pch=19, col='grey82')
  	points(x=x[MS], y=ys[MS,i], pch=2, col='grey82')
  	points(x=x[M], y=ys[M,i], pch=19)
  	lines(stats::lowess(data.frame(x=x[c(S,MS,M)],y=ys[c(S,MS,M),i])), lty=1)
  	}
 if(i==6){
 	par(mar=c(5,1,1,1))
