...
### PANEL A: Plot Mv treatment vs species biomass ###
...
fig2a
#fig2a + geom_jitter(position = position_jitter(w = 0.1, h = 0))
#ggsave(filename="fig2a.pdf", width = 6, height = 4, units = 'in') #save the plot and define its size




### PANEL B: Plot Mv treatment vs biomass measures ###
...
fig2b <- ggplot(sub, aes(x=as.integer(as.character(mvtrt)), y=biomval, colour=comptrt, shape=comptrt)) + 
  ...
  coord_cartesian(xlim=c(-0.5,6.5)) + scale_x_continuous(breaks=seq(0, 6, 1))
fig2b
#fig2b + geom_jitter(position = position_jitter(w = 0.1, h = 0)) #show the raw data points
...
fig2<-arrangeGrob(ag, 
                    ...
                    nrow=1)
fig2
ggsave(filename="e4Output_figures/fig2.pdf", plot=fig2, width = 4, height = 8.5, units = 'in') #save the plot and define its size
