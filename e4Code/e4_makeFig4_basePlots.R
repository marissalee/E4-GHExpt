...
sub2<-PredAttach(sub1, lme.fig4) #update dataframe with predicted values
#Exploring the role of the empty pots
sub3<-subset(data3, type == 'Mivi' | type == 'CompEmpty') #subset data
sub4<-PredAttach(sub3, lme.fig4.noempty) #update dataframe with predicted values
...
colnames(df.aux)[2:3]<-c('line1','line2')
df1.aux<-data.frame(reorder, df.aux)
df2.aux<-orderBy(~reorder, df1.aux)
#For dataset with no empty pots
result.noempty<-PullPvals(lme.fig4.noempty) #ignore warning message, pull pvals
df.aux.noempty<-NamPval(result.noempty, terms)
colnames(df.aux.noempty)[2:3]<-c('line1','line2')
df1.aux.noempty<-data.frame(reorder, df.aux.noempty)
df2.aux.noempty<-orderBy(~reorder, df1.aux.noempty)
...
for (i in 1:length(measorder)){
  ...
  df1<-orderBy(~mivi, data=df) # need to order by x vals so that regession lines work
  #No empty dataset (just using this to add fit line)
  df<-sub4[sub4$soilmeas == measorder[i],]
  df2<-orderBy(~mivi, data=df) # need to order by x vals so that regession lines work
  
  #SETUP PLOT
  plot(soilval~mivi, df1, axes = FALSE, type = "n", 
       xlab='', ylab='',
       xlim=xlims,
       ylim=ylimlist[[i]])
  box()
  ...
  mtext(df2.aux[i,'line1'], side=3, adj=0.95, line=mtxline[2],cex=cex.terms)
  mtext(df2.aux[i,'line2'], side=3, adj=0.95, line=mtxline[3],cex=cex.terms)
  if(i!=1){
    mtext(shortynames[i], side=3, adj=0.95, line=mtxline[1], cex=cex.shortyn)
  }
  ...
  #Y AXIS
  if(i==1){
    axis(2, at = yatlist[[i]], labels=TRUE, tick=TRUE) #side plot
    mtext(row1yname, side = 2, outer = F, cex = 1, line=2) #side plot
  }
  if(i==2){
    axis(2, at = yatlist[[i]], labels=TRUE, tick=TRUE) #side plot
    mtext(row2yname1, side = 2, outer = F, cex = 1, line=3.5) #side plot
    mtext(row2yname2, side = 2, outer = F, cex = 1, line=2) #side plot
  }
  if(i==5){
    axis(2, at = yatlist[[i]], labels=TRUE, tick=TRUE) #side plot
    mtext(row3yname1, side = 2, outer = F, cex = 1, line=3.5) #side plot
    mtext(row3yname2, side = 2, outer = F, cex = 1, line=2) #side plot
  }
  if(i==3 | i==4 | i==6 | i==7){
    axis(2, at = yatlist[[i]], labels=FALSE, tick=FALSE) #side plot
  }
