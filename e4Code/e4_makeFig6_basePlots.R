#e4_makeFig6_basePlots.R
#Make Figure 6: Mixture plant biomass vs soil measure (sF) - total biomass
...
#SET UP DATAFRAMES
##################################
sub1<-subset(data6, biommeas == "mivi") #subset data
sub2<-PredAttach(sub1, lme.fig6) #update dataframe with predicted values
...
for (i in 1:length(measorder)){
  ...
  mtext(df2.aux[i,'line1'], side=3, adj=0.95, line=mtxline[2],cex=cex.terms) #total
  mtext(df2.aux[i,'line2'], side=3, adj=0.95, line=mtxline[3],cex=cex.terms) #total2
  if(i!=1){
  ...
  #ADD DATA
  points(y=df1$sF,x=df1$biomval, pch=16) #add means
  lines(y=df1$pred, x=df1$biomval, pch=16) #add lme regression line
  
}

#PANEL FOR THE X AXIS TITLE
xname.line1<-"Dry aboveground total plant biomass (g)"
par(mar=c(0,0,0,0))
