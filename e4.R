...
yname1<-'Community biomass (g)'
yname2<-'Soil moisture (%)'
yname3<-expression(paste("Nitrate (",mu,"gN/G)"))
yname4<-expression(paste("Nitrification (",mu,"gN/G"%*%"d)"))  
...
mtext("Competitor treatment", side = 1, outer = F, cex = 1, line=3.2)
...
for (i in 1:dim(resps)[2]){
  ...
  if (i %in% 1) {
    legend(x=5.25,y=107.25, 
           legend=c("No comp", expression(italic(Panicum)),expression(italic(Sorghum)), NA,
                    "No comp fit",expression(paste(italic(Panicum)," fit")),expression(paste(italic(Sorghum)," fit")), NA,
                    "Comm. \nbiomass fit", NA), 
           cex=.8,
...
legend(x=5.75,y=109, 
       legend=c(expression(paste(italic(Microstegium),", No comp")),
