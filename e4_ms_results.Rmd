...
```{r, echo=FALSE}
### Mean table
MEANS<-list(sum.fig2a.mivi, sum.fig2b, sum.fig2c)
meannames<-c('Mivi','RelMivi','Total')
i<-0
s.table<-numeric(0)
...
relmivi<-lme.fig2b$anova
totalbiom<-lme.fig2c$anova
tmp<-rbind(mivi, comp,relmivi, totalbiom)
resps.col<-rep(c('mivi', 'comp','relmivi','totalbiom'), each=3)
terms.col<-rep(row.names(mivi), 4)
...
#Microstegium relative biomass
GreaterThan.fig2ab(PrettyMeans.2levs(sum.fig2b))
```
...
###*e4Output_tables/fig5_lme_mivi_anova.txt* -- Soil measures varied by Mivi and comptrt
...
#write.table(MakeTable(thing[['retab']]), file='e4Output_tables/fig5_lme_mivi_re.txt', sep='\t', row.names=F)
write.table(MakeTable(thing[['anovatab']]), file='e4Output_tables/fig5_lme_mivi_anova.txt', sep='\t', row.names=F)
```

###*e4Output_tables/fig5_lme_relmivi_anova.txt* -- Soil measures varied by RelMivi and comptrt  
```{r, echo=FALSE,warning=FALSE, message=FALSE}
#lme: resp <- relmivi + I(relmivi^2) + comptrt + relmivi:comptrt + (1|bk)

#soil measures
thing<-SoilUnlist(lme.fig5.relmivi)
#MakeTable(thing[['anovatab']])
#write.table(MakeTable(thing[['fetab']]), file='e4Output_tables/fig5_lme_relmivi_fe.txt', sep='\t', row.names=F)
#write.table(MakeTable(thing[['retab']]), file='e4Output_tables/fig5_lme_relmivi_re.txt', sep='\t', row.names=F)
write.table(MakeTable(thing[['anovatab']]), file='e4Output_tables/fig5_lme_relmivi_anova.txt', sep='\t', row.names=F)
```

###*e4Output_tables/fig5_lme_total_anova.txt* -- Soil measures varied by Total  
```{r, echo=FALSE,warning=FALSE, message=FALSE}
#lme: resp <- relmivi + I(relmivi^2) + comptrt + relmivi:comptrt + (1|bk)

#soil measures
thing<-SoilUnlist(lme.fig5.total)
#MakeTable(thing[['anovatab']])
#write.table(MakeTable(thing[['fetab']]), file='e4Output_tables/fig5_lme_total_fe.txt', sep='\t', row.names=F)
#write.table(MakeTable(thing[['retab']]), file='e4Output_tables/fig5_lme_total_re.txt', sep='\t', row.names=F)
write.table(MakeTable(thing[['anovatab']]), file='e4Output_tables/fig5_lme_total_anova.txt', sep='\t', row.names=F)
```
